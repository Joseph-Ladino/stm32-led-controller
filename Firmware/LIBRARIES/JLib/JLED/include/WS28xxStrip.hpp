...
#include "StripAnimationManager.hpp"
...
struct WSStripState {
	...
	StripAnimationManager::AnimationType animation;
...
class WS28xxStrip: StripDataInterface {
public:
...
private:
	...
	StripAnimationManager animManager;
	protected:
...
public:
	...
	// TODO: naming conventions etc
	inline void setEffect(std::string_view name) {
		auto animType = animManager.getAnimationTypeFromName(name);
		animManager.setAnimationType(animType);
		state.animation = animType;
	}

	inline StripAnimationManager::AnimationType getEffect() const {
	...
	inline void update() {
		if (state.animation != StripAnimationManager::NONE) {
			animManager.run(this);
		}
	}
	...
	inline const StripIterator begin() override {
		return beginRawBuffer();
	}
	inline const StripIterator end() override {
		return endRawBuffer();
	}
