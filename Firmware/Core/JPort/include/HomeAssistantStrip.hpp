...
class HomeAssistantStrip: public JLED::DMAStrip<_NUM_PIXELS,
...
private:
	...
	const string DeviceName, BaseTopic, StateTopic, DiscoveryTopic, CommandTopic,
			AvailabilityTopic;
	...
	inline void onMessage(JMQTT::Client &client,
		...
		if (msg.topic == CommandTopic) {
			...
			if (doc.containsKey("color")) {
				...
				if (!(color.containsKey("r") && color.containsKey("g")
						&& color.containsKey("b")))
					return;
...
//		this->client = client;
	...
	inline HomeAssistantStrip(std::string_view deviceName = "STM32_LEDStrip", JETHERNET::NetConfig netConf = {}) :
			LEDBase::DMAStrip(),
			DeviceName(deviceName),
			BaseTopic("homeassistant/light/" + DeviceName), // TODO: change base topic level to "homeassistant" instead of "test"
			StateTopic(BaseTopic + "/state"),
			DiscoveryTopic(BaseTopic + "/config"),
			CommandTopic(StateTopic + "/set"),
			AvailabilityTopic(StateTopic + "/available"),
			netConf(netConf)
	{}
}
;
